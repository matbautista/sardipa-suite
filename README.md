...
**Phases 1–10 are in place** (Section 10 of the plan): scaffold, reliability foundations, first-run setup wizard, auth & tenant scoping, Super Admin agency onboarding, Agency Head's own agency setup, Leads CRUD, record locking, Policies — general, and Life insurance detail forms. Phase 11 onward (other lines' checklists, Manager/Head cross-visibility, dashboards, etc.) aren't built yet. The full spec lives in [`insurance-crm-plan.md`](./insurance-crm-plan.md) (currently Baseline v1.4) and remains the source of truth for everything not yet built.
